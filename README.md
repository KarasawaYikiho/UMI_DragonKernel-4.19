...
五台设备的 KernelSU + SUSFS 及 SukiSU + KPM + SUSFS Actions 产物已完成配置与校验和复核。历史 Original 矩阵产物误启用 KernelSU，现已作废，须在修正后的门禁重新构建。本地 boot 重打包已通过内核回读、尺寸和 AVB 校验。启动、Root 隐藏、硬件、功耗及刷写兼容性尚未验证，因此当前产物不可作为正式刷机包发布。
