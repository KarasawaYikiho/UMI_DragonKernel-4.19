...
6. **CI 是唯一发布构建方。** 本机产物仅用于测试和验证，不得上传到 Releases。
...
正式打包在 GitHub Actions 中执行。本机可以运行同一脚本验证，但其输出不得成为 Releases 资产。
...
发布候选必须由 GitHub Actions 从干净提交构建，并由该次工作流直接上传 Releases；禁止上传本机预编译文件。先发布 `rc`，收集启动日志和硬件矩阵；稳定版只从通过全部门禁的同一提交重建。标签格式：`v<version>-umi-hyperos3`。

CI 发布门禁：

- 工作流检出标签指向的精确提交并重新编译；
- 编译、打包、SHA-256、产物上传处于同一工作流链；
- Release 记录构建提交、工具链提交、配置和工作流运行链接；
- 私密打包输入只能经批准的非公开通道注入，禁止进入 Git、缓存、日志或公开产物元数据；
- 任一设备或变体失败时不得发布不完整 Release。
...
- Windows 盘只用于读取私密输入和下载 CI 已发布的正式产物；本机构建输出留在 WSL 作为验证证据。
