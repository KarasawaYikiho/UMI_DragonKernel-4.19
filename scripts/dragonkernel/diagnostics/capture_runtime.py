...
PROBES = (
    ...
    ("joyose_runtime", "pidof com.xiaomi.joyose 2>/dev/null || true; getprop | grep -iE 'joyose|perf|sched|thermal' || true"),
    ("android_power", "dumpsys power 2>/dev/null || true; dumpsys thermalservice 2>/dev/null || true"),
)
...
def adb_command(serial: str | None, command: str) -> list[str]:
    result = ["adb"]
    if serial:
        result += ["-s", serial]
    return result + ["exec-out", "sh", "-c", command]


def capture(serial: str | None, timeout: int) -> dict[str, object]:
    results: dict[str, object] = {}
    for key, command in PROBES:
        try:
            process = subprocess.run(
                adb_command(serial, command),
...
def main() -> None:
    ...
    data = capture(args.serial, args.timeout)
